...
export async function runImport(repoRoot: string, options: ImportCliOptions = {}): Promise<void> {
  if (options.moveProviderFiles) {
    const message = "`--move-provider-files` is not implemented yet; Threadroot import is non-destructive in 0.1.9.";
    if (options.json) {
  ...
  if (options.consolidate) {
    console.log("note: consolidation is report-only in 0.1.9; visible provider files are never moved automatically.");
  }
}
