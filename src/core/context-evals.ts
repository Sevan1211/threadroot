...
export const DEFAULT_CONTEXT_EVALS: ContextEvalCase[] = [
  ...
  { id: "codex-install", task: "install Threadroot for Codex without visible project files", expectedFiles: ["src/core/codex.ts", "src/commands/codex.ts", "test/codex.test.ts"] },
  { id: "codex-status", task: "show Codex CLI and MCP status", expectedFiles: ["src/core/codex.ts", "src/commands/codex.ts", "src/core/mcp-check.ts", "test/codex.test.ts"] },
  ...
  { id: "import-agents", task: "non destructive import Codex AGENTS files", expectedFiles: ["src/core/init/import.ts", "src/commands/import.ts", "test/init.test.ts"] },
  ...
  { id: "compile-agents", task: "compile Codex AGENTS output", expectedFiles: ["src/core/compile/index.ts", "src/core/compile/adapters/agents.ts", "test/compile.test.ts"] },
  { id: "codex-runner", task: "parse Codex exec JSONL events", expectedFiles: ["src/core/codex.ts", "src/core/loop.ts", "test/codex.test.ts"] },
  ...
  { id: "mcp-resources", task: "expose MCP resources for latest task and index", expectedFiles: ["src/mcp/server.ts", "test/mcp-server.test.ts"] },
  {
    ...
    task: "review the full Threadroot product: loops, MCP tools, skills, connections, indexing, Codex integration, and recursive improvements",
    expectedFiles: [
      "src/core/loop.ts",
      "src/core/improve.ts",
