...
export function threadrootSkillContent(provider: AgentProvider, scope: "project" | "global"): string {
  ...
  return [
    ...
    scopeLine,
    "",
    "Threadroot keeps agent-facing project context and capabilities in `.threadroot/` and exposes them through deterministic CLI/MCP commands. Keep broad context out of the chat until it is task-relevant.",
    "",
    "## Workflow",
    "",
    ...
    "3. At the start of a coding session, run `threadroot start \"<task>\"` to get doctor status, project state, relevant skills, tools, connections, memory, and the command map.",
    "4. If no installed skill fits the task, run `threadroot skills find \"<query>\"` and install only through `threadroot skills add <source> --skill <name>` so the skill is scanned, locked, and stored under `.threadroot/skills/`.",
    "5. If no good external skill exists, use the `create-skill` seed skill and create a project-specific skill under `.threadroot/skills/<name>/SKILL.md`.",
    "6. For repeatable commands, use `threadroot tools detect`, then create minimal tools with `threadroot tools create`. Use `threadroot connections add` for local CLI services; never store secrets.",
    "7. Use `threadroot automation status` before agent-created capabilities. If safe automation is wanted, ask the user to run `threadroot automation approve`; medium/high-risk work still requires explicit human review.",
    "8. Do not create provider-specific files unless the user asks. Use `threadroot expose <agent>` for the Threadroot bootstrap shim or `threadroot skills expose <name|all> --agent <agent>` for installed skill shims.",
    "",
    "## Useful Commands",
    "",
    "```bash",
